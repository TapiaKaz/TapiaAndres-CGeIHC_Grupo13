...
int main()
{
	...
	*/
	...
	glm::mat4 modelauxbase(1.0); //inicializar matriz de modelado para la base con llantas
	glm::mat4 modelauxderecha(1.0); // incializar matriz para llantas traseras
	glm::mat4 modelauxizquierda(1.0); // inicializar matriz para llantas delanteras
	...
	while (!mainWindow.getShouldClose())
	{
		...
		///=============== Continuación para la practica04 
		// --- Construcción de lo que falta de la cabina
		// base de piramide con base cuadrangular:
		...
		model = glm::rotate(model, glm::radians(90.0f), ejeX);
		

		modelauxderecha = model;

		model = glm::rotate(model, glm::radians(mainWindow.getruedaz()), ejeY);
		...
		model = glm::rotate(model, glm::radians(mainWindow.getruedax()), ejeY);
		...
		model = glm::rotate(model, glm::radians(mainWindow.getruedac()), ejeY);
		...
		model = glm::rotate(model, glm::radians(mainWindow.getruedav()), ejeY);
	...
	//primer brazo que conecta con la cabina
		model = glm::rotate(model, glm::radians(135.0f), ejeZ);
		model = glm::translate(model, glm::vec3(2.5f, 0.0f, 0.0f));
		modelaux = model;
		...
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion4()), ejeY);
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion5()), ejeY);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		sp.render();
