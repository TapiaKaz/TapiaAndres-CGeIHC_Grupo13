...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	theWindow->movimientoAuto = 0.0f;// reiniciamos el estado del auto a cero (no se esta moviendo)
	
	if (key == GLFW_KEY_Y && action != GLFW_RELEASE)
	{
		theWindow-> muevex += 1.0;
		theWindow-> movimientoAuto = -1.0f; // cuando el auto se mueva para adelante su movimiento sera "positivo"
	}
	if (key == GLFW_KEY_U && action != GLFW_RELEASE)
	{
		theWindow-> muevex -= 1.0;
		theWindow-> movimientoAuto = +1.0f; // cuando el auto se mueva para atras su movimiento sera "negativo"
	}
